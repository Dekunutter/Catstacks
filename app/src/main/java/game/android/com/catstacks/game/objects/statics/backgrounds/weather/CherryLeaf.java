...
public class CherryLeaf extends BackgroundScenery
{
    private float x, y, depth, originalY, floatTimer;
    private float horizontalFloat;

    public final static GenericObjectPool<CherryLeaf> pool = new GenericObjectPool<CherryLeaf>(CherryLeaf.class);
    ...
    public CherryLeaf(Texture texture, Vec position)
    {
        depth = - 6;
    ...
    public void update()
    {
        ...
        {
            body.move(body.getX() + horizontalFloat, body.getY() - (5 * getMovementDeltaY()), rotation);
            body.setX(body.getX() + horizontalFloat);
            body.setY(body.getY() - (5 * getMovementDeltaY()));
        }
        ...
        if(body.getY() <= 0)
        {
            ...
            body.move(body.getX() - spawnPosition.x, body.getY() - newY, rotation);
            body.setX(spawnPosition.x);
            body.setY(newY);*/
            remove();
        }
    }
