...
public class BeachBall extends BackgroundScenery
{
    ...
    public BeachBall(Vec position)
    {
        addSpriteToPosition(R.drawable.beach_ball, 4, 1, -4f, 0, position);
    ...
    private void initAnimations()
    {
        ...
        lighting.init(BOUNCING_ANIM, 0, frames, 6);
    ...
    public void update()
    {
        ...
        else
        {
            ...
            if(bouncing)
            {
                movementX = Math.min(movementX + (getMovementDeltaX() * 0.75f), 2.5f * getMovementDelta());
                newX = body.getX() + movementX;
            }
            else
            {
                movementX = 2.5f * getMovementDelta();
                newX = body.getX() + movementX;
            }
        }
        ...
        else
        {
            if(bouncing)
            {
                movementY = Math.min(movementY + (getMovementDeltaY() * 0.5f), 5 * getMovementDelta());
                newY = body.getY() + movementY;
            }
            else
            {
                movementY = Math.max(movementY + (getMovementDeltaY() * -0.5f), -5 * getMovementDelta());
                newY = body.getY() + movementY;
            }
        }
