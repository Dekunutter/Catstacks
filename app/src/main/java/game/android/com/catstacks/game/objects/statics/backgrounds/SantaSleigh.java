...
public class SantaSleigh extends BackgroundScenery
{
    private float originalX, floatTimer, depth;
    private int floatFrame;

    private final static int GALLOP_FRAME_1 = 0;
    private final static int GALLOP_FRAME_2 = 1;

    private final static String GALLOP_ANIM = "gallop";

    public SantaSleigh(Vec position)
    {
        depth = -6.5f;
        addSprite(R.drawable.santa_sleigh, 1, 2, depth, 0);

        activeFrames.add(textures.get(0).getFrame(GALLOP_FRAME_1));

        initAnimations();
        getAnimation(GALLOP_ANIM).activate();
    ...
    private void initAnimations()
    {
        ...
        frames.add(textures.get(0).getFrame(GALLOP_FRAME_1));
        frames.add(textures.get(0).getFrame(GALLOP_FRAME_2));
        Animation galloping = Animation.pool.get();
        galloping.init(GALLOP_ANIM, 0, frames, 3);

        animations.add(galloping);
    }
